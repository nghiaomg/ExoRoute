...
// The release carries the upstream public desktop OAuth client as a fallback.
// These environment variables remain available for an operator-supplied client.
...
use public_credentials::{client_id as bundled_client_id, client_secret as bundled_client_secret};
...
fn oauth_client_secret() -> Result<Option<String>, String> {
    Ok(Some(resolve_public_oauth_client_value(
        CLIENT_SECRET_ENV,
...
fn resolve_public_oauth_client_value(
    env_name: &str,
    configured: Option<String>,
    bundled: String,
) -> Result<String, String> {
    let configured = resolve_optional_oauth_client_value(env_name, configured)?;
    resolve_required_oauth_client_value(env_name, configured.or(Some(bundled)))
}
