...
#[test]
fn bundled_public_oauth_client_is_available_without_environment() {
    let client_id = super::public_credentials::client_id().expect("bundled client id");
    let client_secret = super::public_credentials::client_secret().expect("bundled client secret");

    assert!(client_id.ends_with(".apps.googleusercontent.com"));
    assert!(client_secret.starts_with("GOCSPX-"));
    assert_eq!(
        resolve_public_oauth_client_value(CLIENT_ID_ENV, None, "bundled-client-id".to_owned())
            .unwrap(),
        "bundled-client-id"
    );
    assert_eq!(
        resolve_public_oauth_client_value(
            CLIENT_ID_ENV,
            Some("  custom-client-id  ".to_owned()),
            "bundled-client-id".to_owned(),
        )
        .unwrap(),
        "custom-client-id"
    );
...
#[test]
fn token_exchange_sends_public_secret_without_pkce_verifier() {
    let form = super::oauth::token_exchange_form(
        "client-id".to_owned(),
    ...
    assert_eq!(value("client_secret"), Some("client-secret"));
    assert_eq!(value("code_verifier"), None);
