<script lang="ts">
  ...
  import { Activity, CircleCheck, CircleX, Clock3, RefreshCw } from '@lucide/svelte';
...
import { localizedError } from '../../lib/errors';
  ...
  export let apiKey: GatewayApiKey | null = null;
  ...
  $: if (apiKey && apiKey.id !== loadedKeyId) {
  ...
  async function load(key: GatewayApiKey, range: GatewayApiKeyStatistics['range']): Promise<void> {
    selectedRange = range;
    ...
    errorMessage = '';
    try {
      const result = await api.apiKeyStatistics(key.id, range, controller.signal);
...
<ArkDialog open={apiKey !== null} closeLabel={tr('Close dialog')} title={apiKey?.name ?? tr('API key usage')} kicker={tr('EXOROUTE CONTROL PLANE')} wide onClose={close}>
  {#if apiKey}
    <div class="modal-form">
      <p class="modal-description">{tr('Traffic for {name} in the selected period.', { name: apiKey.name })}</p>
      <div class="api-key-details-range" role="group" aria-label={tr('Statistics time range')}>
        <button type="button" class:active={selectedRange === '1d'} aria-pressed={selectedRange === '1d'} disabled={isLoading} onclick={() => load(apiKey, '1d')}>{tr('24 hours')}</button>
        <button type="button" class:active={selectedRange === '7d'} aria-pressed={selectedRange === '7d'} disabled={isLoading} onclick={() => load(apiKey, '7d')}>{tr('7 days')}</button>
        <button type="button" class:active={selectedRange === '30d'} aria-pressed={selectedRange === '30d'} disabled={isLoading} onclick={() => load(apiKey, '30d')}>{tr('30 days')}</button>
      </div>
      ...
      {:else if errorMessage && !statistics}
        ...
        <div class="modal-actions"><button type="button" class="secondary-button" onclick={() => load(apiKey, selectedRange)}>{tr('Retry')}</button></div>
      {:else if statistics}
        ...
        {:else}
          <p class="donut-empty">{tr('No traffic for this API key in the selected period.')}</p>
        {/if}
        {#if errorMessage}
          <div class="form-error" role="alert">{errorMessage}</div>
        {/if}
        <div class="modal-actions">
          <button type="button" class="secondary-button compact" disabled={isLoading} onclick={() => load(apiKey, selectedRange)}>
            {#if isLoading}<span class="auth-bootstrap-spinner"></span>{:else}<RefreshCw size={14} />{/if}{tr('Refresh')}
          </button>
          <button type="button" class="primary-button" onclick={close}>{tr('Done')}</button>
        </div>
      {/if}
    </div>
  {/if}
...
<style>
  ...
  .api-key-details-kpis { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; margin-bottom: 14px; }
  ...
  .api-key-details-tokens { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 1px; overflow: hidden; margin-bottom: 14px; border: 1px solid var(--line); border-radius: 13px; background: var(--line); }
  ...
  .api-key-details-warning { display: flex; align-items: flex-start; gap: 10px; margin-bottom: 12px; padding: 12px 14px; color: #9d4827; border: 1px solid #f3d1bf; border-radius: 10px; background: #fff8f4; }
